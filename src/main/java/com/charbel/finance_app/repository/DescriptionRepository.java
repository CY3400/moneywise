...
public interface DescriptionRepository extends JpaRepository<Description, Long> {
    @Query(value="SELECT * FROM DESCRIPTION WHERE (LOWER(DESCRIPTION) LIKE CONCAT('%', LOWER(:description), '%') OR :description IS NULL) AND (TYPE IN (:type,3) OR :type IS NULL) ORDER BY DESCRIPTION",nativeQuery = true)
